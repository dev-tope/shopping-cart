...
const ShopPage = ({cart, setCart,click, onAddToCart, isCartOpened }) => {
  ...
  if(error) return <p className={`${styles.error}`}>There was an error loading resources</p>
    ...
    <div className={`${styles.shopDiv}`}>
      <div className={`${styles.left}`}>
        <div className={`${styles.leftNavDiv}`}>
          <nav className={`${styles.navShopPage}`}>
      ...
      { isCartOpened &&
        <div id="cart-div">
          <CartPage cart={cart} setCart={setCart} click={click} />
        </div>
      }
    </div>
  )
